...
void endMove(int exit)
{
    for (int i = 0; i < 3; i ++) {
        if (mems[i].rewrite) {
            ...
            if (exit) printf("MOV [%d] r%d\n", 4 * i, i);
        }
        if (regs[i].isVac) {
            regs[i].val = mems[i].val;
            if (exit) printf("MOV r%d [%d]\n", i, 4 * i);
        }
    }
...
void genCode(BTNode *cur)
{
    ...
    if (isdigit(c)) {
        ...
        if (varReg(cur -> left) != size - 1) {
            regs[varReg(cur -> left)].val = regs[size - 1].val;
            regs[size - 1].isVac = 1;
            if (regs[varReg(cur -> left)].assigned) {
                printf("MOV r%d r%d\n", varReg(cur -> left), size - 1);
            }
            else printf("MOV [%d] r%d\n", 4 * varReg(cur -> left), size - 1);
        }
        regs[varReg(cur -> left)].isVac = 0;
        mems[varReg(cur -> left)].rewrite = 1;
        regs[0].assigned = regs[1].assigned = regs[2].assigned = 0;
        for (int i = 0; i < 8; i ++) {
            if (regs[i].isVac) {
                size = i;
                break;
...
void calNode(BTNode *cur)
{
    ...
    else if (isalpha(c) && mems[c - 'x'].rewrite && regs[c - 'x'].assigned == 0) {
...
void statement(void)
{
    ...
    if (match(END)) {
        ...
        if (match(END)) {
            checkSyntax();
            regs[varReg(retp -> left)].assigned = 1;
            checkMath(retp);
            calNode(retp);
            genCode(retp);
            //            printInfix(retp);
