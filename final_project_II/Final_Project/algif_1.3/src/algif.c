 ...
 */
int
algif_load_animation (const char *filename, ALLEGRO_BITMAP ***frames, int **durations)
{
    GIF_ANIMATION *gif = algif_load_raw_animation (filename);
    int i, n;
    ALLEGRO_BITMAP *prev = NULL;
    ...
    *frames = calloc (n, sizeof **frames);
    ...
    for (i = 0; i < n; i++)
    {
        ALLEGRO_BITMAP *b = (*frames)[i] = create_bitmap (gif->width, gif->height);
        if (prev)
...
ALLEGRO_BITMAP *
load_gif (const char *filename, ALLEGRO_COLOR *pal)
{
    int i;
    GIF_ANIMATION *gif = algif_load_raw_animation (filename);
    ALLEGRO_BITMAP *bmp = NULL;
    GIF_PALETTE gifpal;
    ALLEGRO_PALETTE tmppal;
