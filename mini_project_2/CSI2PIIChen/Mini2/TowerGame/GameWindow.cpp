...
GameWindow::game_init()
{
    ...
    atk_sldr = new Slider(window_width * 0.36, window_height * 6/7);
    atk_sldr -> set_color(al_map_rgb(0, 0, 0));
    atk_sldr -> set_label_content("Volume");
}
...
GameWindow::GameWindow()
{
    ...
    Medium_font = al_load_ttf_font("Caviar_Dreams_Bold.ttf",36,0); //load medium font
    Large_font = al_load_ttf_font("Caviar_Dreams_Bold.ttf",60,0); //load large font
...
//    unsigned int j;
    ...
    for (auto t : towerSet) {
        for (auto m : monsterSet) {
            canAttack = t -> DetectAttack(m);
            if (canAttack) attack = t -> TriggerAttack(m);
        }
    }
    ...
    for(i=0; i < monsterSet.size(); i++)
    {
        ...
        for (auto t : towerSet) {
            if (t -> TriggerAttack(monsterSet[i])) {
                isDestroyed = true;
                break;
            }
        }
...
//    int offsetY = field_height/2 - 200;
    ...
    if (windowState == STATE_START) {
        if (event.type == ALLEGRO_EVENT_KEY_DOWN) {
            if (event.keyboard.keycode == ALLEGRO_KEY_ENTER) {
                windowState = STATE_GAME;
                redraw = true;
            }
        }
        ...
        if (change) {
            if (atk_sldr -> isClicked(event.mouse.x, event.mouse.y)) {
                float vol = atk_sldr -> Drag(event.mouse.x, event.mouse.y);
                set_attack_volume(vol);
                al_set_sample_instance_gain(backgroundSound, Attack::volume);
                atk_sldr -> Draw();
                redraw = true;
            }
        }
    }
    else if (windowState ==STATE_GAME) {
    ...
    else if (windowState == STATE_PAUSE) {
        al_stop_sample_instance(backgroundSound);
        if (event.type == ALLEGRO_EVENT_KEY_DOWN) {
...
GameWindow::draw_running_map()
{
    if (windowState == STATE_START) {
        ...
        al_draw_text(Large_font, al_map_rgb(255, 0, 0), window_width / 5, window_height / 3, ALLEGRO_ALIGN_CENTER, "SUPER");
        al_draw_text(Large_font, al_map_rgb(0, 0, 255), window_width / 2 - 20, window_height / 3, ALLEGRO_ALIGN_CENTER, "TOWER");
        al_draw_text(Large_font, al_map_rgb(22, 127, 19), window_width * 4/5, window_height / 3, ALLEGRO_ALIGN_CENTER, "DEFENSE");
        al_draw_text(Large_font, al_map_rgb(0, 0, 0), window_width / 2, window_height / 2, ALLEGRO_ALIGN_CENTER, "!!!!!");
        al_draw_text(Medium_font, al_map_rgb(255, 255, 255), window_width/2, window_height * 2 / 3, ALLEGRO_ALIGN_CENTER, "Press 'Enter' to start");
        ...
        al_draw_text(Large_font, al_map_rgb(255, 255, 255), window_width * 2/5, window_height / 3, ALLEGRO_ALIGN_CENTER, "PAUSED");
    }
    else show_err_msg(39);
